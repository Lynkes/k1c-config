...
class CUSTOM_MACRO:
    ...
    def get_status(self, eventtime):
        return {
            ...
            'extruder_temp': self.extruder_temp if self.extruder_temp is not None else self.default_extruder_temp,
            'bed_temp': self.bed_temp if self.bed_temp is not None else self.default_bed_temp,
    ...
    def cmd_CX_NOZZLE_CLEAR(self, gcmd):
        bed_temp = gcmd.get_float('BED_TEMP', default=self.bed_temp if self.bed_temp is not None else self.default_bed_temp, minval=0.0, maxval=130.0)
        extruder_temp = gcmd.get_float('EXTRUDER_TEMP', default=self.extruder_temp if self.extruder_temp is not None else self.default_extruder_temp, minval=140.0, maxval=320.0)
        fan_speed = gcmd.get_int('FAN_SPEED', default=255, minval=0, maxval=255)
        self.bed_temp = bed_temp
        self.extruder_temp = extruder_temp
        self.gcode.run_script_from_command('NOZZLE_CLEAR HOT_MIN_TEMP=%d HOT_MAX_TEMP=%d BED_MAX_TEMP=%d FAN_SPEED=%d' % (self.g28_ext_temp, extruder_temp - 20, bed_temp, fan_speed))
        pass
